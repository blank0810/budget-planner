            ...
            <div class="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-3 mb-4">
                ...
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                    <div class="p-6 bg-white border-b border-gray-200">
                        <div class="flex items-center">
                            <div
                                class="flex-shrink-0 {{ $balance >= 0 ? 'bg-green-500' : 'bg-red-500' }} rounded-md p-3">
                                <svg class="h-6 w-6 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            ...
                            <div class="ml-5 w-0 flex-1">
                                <dl>
                                    ...
                                    <dd class="flex items-baseline">
                                        <div
                                            class="text-2xl font-semibold {{ $balance >= 0 ? 'text-green-600' : 'text-red-600' }}">
                    ...
                    @if($budgets->isNotEmpty())
                                    <div class="mb-6">
                                        <div class="flex justify-between mb-2">
                                            <span class="text-sm font-medium text-gray-700">Total Budgeted:
                                                ${{ number_format($budgetSummary->total_budgeted, 2) }}</span>
                                            <span
                                                class="text-sm font-medium {{ $budgetSummary->status === 'over' ? 'text-red-600' : 'text-gray-700' }}">
                                                Spent: ${{ number_format($budgetSummary->total_spent, 2) }}
                                                ({{ number_format($budgetSummary->utilization, 1) }}%)
                                            </span>
                                        </div>
                        ...
                        ($budgetSummary->status === 'warning' ? 'bg-yellow-500' : 'bg-green-500') 
                                                ...
                                                style="width: {{ min(100, $budgetSummary->utilization) }}%">
                                            </div>
                                        </div>
                                    </div>
                                        ...
                                        @foreach($budgets as $budget)
                                                        <div>
                                                            <div class="flex justify-between text-sm mb-1">
                                                                ...
                                                                <span
                                                                    class="font-medium {{ $budget->status === 'over' ? 'text-red-600' : 'text-gray-700' }}">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <div class="bg-white overflow-hidden shadow-sm rounded">
                    <div class="p-4">
                                    ...
                                    <tbody class="bg-white divide-y divide-gray-200">
                                        @foreach($recentTransactions as $transaction)
                                            <tr>
                                                ...
                                                <td
                                                    class="px-6 py-4 whitespace-nowrap text-right text-sm {{ $transaction instanceof \App\Models\Income ? 'text-green-600' : 'text-red-600' }}">
                        ...
                        @else
                            <p class="text-gray-500 text-center py-4">No transactions yet. Add some income or expenses to
                                get started.</p>
                        @endif
                    </div>
                </div>
